...
static char highscore_buffer[32];
...
static void update_highscore() {
	snprintf(highscore_buffer, sizeof(highscore_buffer), "%d", highscore);
}
	...
	if (curr_direction == 1 && currx < boundx + ball_radius) {
		...
		if (curr_direction == 0 && currx > 0 - ball_radius - 1) {
	    	...
	    	if (persist_exists(HIGHSCORE_KEY)) {
				highscore = persist_read_int(HIGHSCORE_KEY);
		...
		for (i = 0; i < hole_list[9].wall_side; i++) { 
			if (hole_list[i].wall_side == 999) {
				...
				if (hole_list[i].wall_side != 999) {
					...
					int j = 0;
					bool ok = 1;
					for (j = 0; j < hole_list[9].wall_side; j++) {
						if (i!=j) {
...
static void game_draw(GContext *ctx) {
  // Per-frame game rendering here
	...
	if (show_final_score == 0) {
		...
		graphics_draw_text(ctx, "HIGHSCORE:" , fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD), GRect(10, 90 , boundx - 20, 20), GTextOverflowModeFill, GTextAlignmentCenter, NULL);
		graphics_draw_text(ctx, highscore_buffer, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD), GRect(10, 110 , boundx - 20, 20), GTextOverflowModeFill, GTextAlignmentCenter, NULL);
}
...
static void splash_done_handler() {
	...
	curry = boundy - ball_radius - MARGIN - 1;
